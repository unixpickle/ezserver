...
func (h *HTTPS) HandleAutocertRequest(w http.ResponseWriter, r *http.Request) bool {
	log.Printf("handling autocert request %v %s %s", w.Header(), r.URL, r.RemoteAddr)
	h.managerLock.RLock()
	manager := h.manager
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		log.Printf("handling autocert request failed %v %s %s", w.Header(), r.URL, r.RemoteAddr)
		handled = false
	}
	manager.HTTPHandler(http.HandlerFunc(handler)).ServeHTTP(w, r)
	if handled {
		log.Printf("handling autocert request succeeded %v %s %s", w.Header(), r.URL, r.RemoteAddr)
	}
	return handled
}
